...
void ofApp::update() {
	...
	for (int v = 0; v < 360; v += v_span) {
		...
		for (auto u = 0; u < 360; u += u_span) {
			...
			auto noise_value = ofNoise(glm::vec4(this->make_point(R, r, u + u_span * 0.5, v) * 0.005, ofGetFrameNum() * 0.01));
...
void ofApp::draw() {
	...
	for (int i = 0; i < number; i++) {
		...
		location_list[i] = this->make_point(R, r, u, v);
	}
